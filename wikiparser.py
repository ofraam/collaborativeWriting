...
class MIP:
    ...
    def updateMIP(self, newVersion):
        #clear updated edges
        for edge in self.mip.edges_iter():
            edge['updated']=0
            
        #TODO: code for adding new pars and users, and updating weights
        mappings = generate_mapping_for_revision(self.currentVersion,newVersion)
        
        
        #decay weights
        for edge in self.mip.edges_iter():
            edge['weight']=max(0,edge['weight']-self.decay)
    ...
    def addPars(self,parPrevIndex,parNewIndex):
        if (parPrevIndex is not None):
            nodeId=self.pars[parPrevIndex]
            self.mip[nodeId][self.latestVersion]=parNewIndex
        else:
            self.lastID=self.lastID+1
            self.pars[parNewIndex] = self.lastID
            attr = {}
            attr['type']='par'
            attr[self.latestVersion]=parNewIndex
            self.mip.add_node(self.lastID, attr)
            
    def updateEdge(self,i1,i2,increment = 1):
...
def generate_mapping_for_revision(v1,v2):
    m_f = assign_neighbors(v1, v2, .4)
    m_b = assign_neighbors(v2, v1, .4)
...
newer = os.walk("newest_predictions").next()[2]
...
for a in os.walk("pickles"):
    for b in a[2]:
        if b not in newer and not "ofra" in b:
            ...
            prediction_file = os.path.join(os.getcwd(), "newest_predictions", pickle_file_name)
