...
class prediction_pickle():
    def __init__(self):
        ...
        self.authors_sig_changes = None
        self.anon_authors_with_sig = None
        self.named_authors_with_sig = None
    ...
    for b in a[2]:
        if b not in newer and not "ofra" in b:
            ...
            if has_authors:
                ...
                #author significant changes
                versions_sig_changes = []
                for i in find_significant_changes(.9).keys():
                    versions_sig_changes.append(i)

                authors_sig_changes = {}
                for a in versions_sig_changes:
                    author_of_a = current_names[a]
                    authors_sig_changes[author_of_a] = authors_sig_changes.get(author_of_a,0) + 1
